...
export function PlaygroundClient() {
  ...
  async function run() {
    ...
    try {
      ...
      const firstValid = sample.items.find((v) => typeof v.id === 'number' && typeof v.name === 'string' && typeof v.active === 'boolean') ?? sample.items[0];
      try { ZodUser.parse(firstValid); } catch {}
      ...
      for (let i = 0; i < runs; i++) {
        ...
        for (let j = 0; j < sample.items.length; j++) { try { ZodUser.parse(sample.items[j]); } catch {} }
        const z1 = performance.now();
        zodTimes.push(z1 - z0);
      }

      setData([{ label: "median", dhi: median(dhiTimes), zod: median(zodTimes) }]);
    } finally {
      setRunning(false);
    }
  }
    ...
    <div className="py-10">
      ...
      <div className="mt-4 flex flex-wrap gap-3 items-end">
        <label className="text-sm">Sample size
          <input type="number" className="ml-2 border rounded px-2 py-1 w-28" value={count} min={1000} max={200000} step={1000} onChange={(e) => setCount(parseInt(e.target.value || "0", 10))} />
        </label>
        <label className="text-sm">Invalids
          <select className="ml-2 border rounded px-2 py-1" value={invalidRatio} onChange={(e) => setInvalidRatio(parseFloat(e.target.value))}>
        ...
        <label className="text-sm">Runs
          <input type="number" className="ml-2 border rounded px-2 py-1 w-20" value={runs} min={1} max={15} onChange={(e) => setRuns(parseInt(e.target.value || "0", 10))} />
        </label>
      ...
      <p className="mt-3 text-xs text-ink-2">Notes: Runs in your browser tab. DHI uses batch fast paths; Zod parses serially. Results vary by hardware and sample.</p>
    </div>
  );
}
...
function makeSample(n: number, invalidRatio: number) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    items[i] = isInvalid ? { id: String(i), name: i, active: "yes" } : { id: i, name: `user-${i}`, active: (i & 1) === 0 };
  }
  return { items };
}
