...
const rows: Row[] = [
  { scenario: "Simple 4-Field (benchmark2.ts)", size: "1,000,000", dhiMs: 58.02, dhiOps: 17235938.549, zodMs: 61.8, zodOps: 16182176.902, speedup: 1.07 },
  { scenario: "Nested Object", size: "100,000", dhiMs: 9.11, dhiOps: 10979509.37, zodMs: 17.32, zodOps: 5773027.723, speedup: 1.9 },
  { scenario: "Array-Heavy", size: "50,000", dhiMs: 9.07, dhiOps: 5511430.978, zodMs: 28.97, zodOps: 1725761.289, speedup: 3.19 },
  { scenario: "Mixed Valid/Invalid", size: "500,000", dhiMs: 30.41, dhiOps: 16441838.23, zodMs: 661.28, zodOps: 756110.688, speedup: 21.75 },
];
    ...
    <div className="py-10">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="text-ink-2 mt-2">Average 6.98× (range 1.07×–21.75×). Transparent and reproducible.</p>
        </div>
        <div className="flex items-center gap-2 text-sm" role="group" aria-label="Toggle units">
          <button className={`px-3 py-1.5 rounded-md border ${unit === "ms" ? "bg-primary text-white" : "bg-white"}`} onClick={() => setUnit("ms")} aria-pressed={unit === "ms"}>ms</button>
          <button className={`px-3 py-1.5 rounded-md border ${unit === "ops" ? "bg-primary text-white" : "bg-white"}`} onClick={() => setUnit("ops")} aria-pressed={unit === "ops"}>ops/sec</button>
        </div>
      </div>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.scenario} className="border-t">
                ...
                <td className="p-3 align-top"><Metric unit={unit} ms={r.dhiMs} ops={r.dhiOps} color="var(--orange)" /></td>
                <td className="p-3 align-top"><Metric unit={unit} ms={r.zodMs} ops={r.zodOps} color="#9CA3AF" /></td>
          ...
          <div className="mt-3 h-[360px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={rows} margin={{ left: 10, right: 10 }}>
                ...
                <Tooltip formatter={(val: unknown) => { const n = typeof val === "number" ? val : Number(val); return unit === "ms" ? `${n.toFixed(2)} ms` : `${n.toLocaleString()} ops/s`; }} />
                <Legend />
                {unit === "ms" ? (
                  <>
      ...
      <section className="mt-10 rounded-xl border p-5">
        ...
        <div className="mt-4 flex gap-3 text-sm">
          <a className="underline" href="/benchmarks.csv" download>Download raw CSV</a>
          <a className="underline" href="https://github.com/" target="_blank" rel="noreferrer">Open benchmark repo</a>
